...
public class CustomerContactInfoCommandTests
{
    ...
    [Fact]
    public async Task InsertCustomerContactInfoCommand_Validation_Error()
    {
    ...
    [Fact]
    public async Task InsertCustomerContactInfoCommand_Validation_Error_Count()
    {
        var mockUnitOfWork = GetMockUnitOfWork(returnValues: new RepositoryReturnValues());
        mockUnitOfWork.Setup(x => x.CustomerContactInfoRepository.InsertAsync(It.IsAny<CustomerContactInfo>())).ReturnsAsync((SqlResult?)null);
        mockUnitOfWork
            .Setup(x => x.CustomerContactInfoRepository.GetCountByCustomerIdAsync(It.IsAny<int>()))
            .ReturnsAsync(5);
        var command = new InsertCustomerContactInfoCommand
        {
            Value = "+46704512343",
        ...
        Assert.Collection(errors, error => Assert.Equal("Contact info count", error.PropertyName));
        mockUnitOfWork.VerifyGet(x => x.CustomerContactInfoRepository, Times.Once);
        mockUnitOfWork.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Never);
        mockUnitOfWork.Verify(x => x.CustomerContactInfoRepository.InsertAsync(It.IsAny<CustomerContactInfo>()), Times.Never);
        mockUnitOfWork.Verify(x => x.CustomerContactInfoRepository.GetCountByCustomerIdAsync(It.IsAny<int>()), Times.Once);
    }
    ...
    [Fact]
    public async Task UpdateCustomerContactInfoCommand_Validation_Error()
    {
        var mockUnitOfWork = GetMockUnitOfWork(returnValues: new RepositoryReturnValues());
    ...
    [Fact]
    public async Task DeleteCustomerContactInfoCommand_Validation_Error()
    {
        var mockUnitOfWork = GetMockUnitOfWork(returnValues: new RepositoryReturnValues());
        mockUnitOfWork
            .Setup(x => x.CustomerContactInfoRepository.GetCountByCustomerIdAsync(It.IsAny<int>()))
        ...
        mockUnitOfWork.Verify(x => x.CustomerContactInfoRepository.GetByIdAsync(It.IsAny<int>()), Times.Never);
        mockUnitOfWork.Verify(x => x.CustomerContactInfoRepository.GetCountByCustomerIdAsync(It.IsAny<int>()), Times.Never);
    }

    [Fact]
    public async Task DeleteCustomerContactInfoCommand_Validation_Error_Count()
    {
        var mockUnitOfWork = GetMockUnitOfWork(returnValues: new RepositoryReturnValues());
        ...
        Assert.Single(errors);
        Assert.Collection(errors, error => Assert.Equal("Contact info count", error.PropertyName));
        mockUnitOfWork.VerifyGet(x => x.CustomerContactInfoRepository, Times.Exactly(2));
        mockUnitOfWork.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Never);
        mockUnitOfWork.Verify(x => x.CustomerContactInfoRepository.DeleteByIdAsync(It.IsAny<int>()), Times.Never);
        mockUnitOfWork.Verify(x => x.CustomerContactInfoRepository.GetByIdAsync(It.IsAny<int>()), Times.Once);
        mockUnitOfWork.Verify(x => x.CustomerContactInfoRepository.GetCountByCustomerIdAsync(It.IsAny<int>()), Times.Once);
